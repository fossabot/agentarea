      ...
      try {
        ...
        if (isUUID) {
          ...
          if (error) {
            ...
            console.warn('Failed to fetch model by UUID, trying string search:', error);
          } else if (data) {
            setModelInfo({
        ...
        const { data: modelSpecs, error } = await listModelSpecs({ is_active: true });
        ...
        if (modelSpecs) {
          ...
          const foundModel = modelSpecs.find(spec => 
