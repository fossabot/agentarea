...
export const createAgent = async (
  agent: components["schemas"]["AgentCreate"]
) => {
  const { data, error } = await browserClient.POST("/v1/agents/", {
    body: agent,
  });
  return { data, error };
};
...
export const getAllTasks = async () => {
  try {
    ...
    for (const agent of agents) {
      const { data: agentTasks, error: tasksError } = await listAgentTasks(
        agent.id
      );
...
export const sendMessage = async (
  ...
  const { data, error } = await browserClient.POST("/v1/chat/messages", {
    body: message,
  });
  return { data, error };
};
...
export const createMCPServer = async (
  ...
  const { data, error } = await browserClient.POST("/v1/mcp-servers/", {
    body: server,
  });
  return { data, error };
};
...
export const createProviderConfig = async (
  ...
  const { data, error } = await browserClient.POST("/v1/provider-configs/", {
    body: config,
  });
  return { data, error };
};
...
export const updateProviderConfig = async (
  ...
  const { data, error } = await browserClient.PUT(
...
export const createModelSpec = async (
  spec: components["schemas"]["ModelSpecCreate"]
) => {
  const { data, error } = await browserClient.POST("/v1/model-specs/", {
    body: spec,
  });
  return { data, error };
};
...
export const upsertModelSpec = async (
  spec: components["schemas"]["ModelSpecCreate"]
) => {
  const { data, error } = await browserClient.POST("/v1/model-specs/upsert", {
    body: spec,
  });
  return { data, error };
};
...
export const createModelInstance = async (
  ...
  const { data, error } = await browserClient.POST("/v1/model-instances/", {
    body: instance,
  });
  return { data, error };
};
