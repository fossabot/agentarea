  ...
  const handleTestLLM = async (e: React.MouseEvent) => {
    ...
    try {
      const { data, error } = await testModelInstance({
        ...
        test_message:
          "Hello, this is a test message to verify the LLM configuration.",
      });

      if (error || !data) {
        const errorMessage =
          error?.detail?.[0]?.msg || (error as any)?.message || "Test failed";
        setTestResult({
          success: false,
          ...
          {testResult && (
            ...
            >
              ...
              {testResult.success && testResult.cost && (
                <div className="mt-1 text-green-600">
                  Cost: ${testResult.cost.toFixed(6)} ({testResult.tokens_used}{" "}
                  tokens)
                </div>
              )}
