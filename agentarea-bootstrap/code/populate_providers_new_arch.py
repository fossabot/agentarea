...
def upsert_provider_spec(
    ...
    # for bootstrap/system-level data.
    # These are built-in providers available to all workspaces, created
    # during system initialization before any users/workspaces exist.
    # This is acceptable for bootstrap scripts.
    conn.execute(
        text("""
            INSERT INTO provider_specs
            (id, provider_key, name, description, provider_type, icon,
             is_builtin, created_by, workspace_id, created_at, updated_at)
            VALUES (:id, :provider_key, :name, :description, :provider_type,
                    :icon, :is_builtin, :created_by, :workspace_id,
                    now(), now())
        """),
        {
            "id": provider_id,
...
def upsert_model_spec(
    ...
    # NOTE: Using "system" for workspace_id and created_by is intentional
    # for bootstrap/system-level data.
    # These are built-in model specs available to all workspaces, created
    # during system initialization before any users/workspaces exist.
    # This is acceptable for bootstrap scripts.
